...
APP=pantheon-sideload
BIN="io.elementary.sideload" #CHANGE THIS IF THE NAME OF THE BINARY IS DIFFERENT FROM "$APP" (for example, the binary of "obs-studio" is "obs")
DEPENDENCES="nss-mdns" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="gdk-pixbuf libjson-glib libXcursor libcrypto.so liblz libnghttp libidn libssh libpsl.so libgssapi libbrotlidec libgpgme.so libsodium.so \
libavahi libcomposefs libblkid.so libthai.so libfreetype.so libxcb libpixman libgraphite libexpat.so libsqlite libjbig.so libicudata.so \
libgstplay libmedia libdav libgnutls.so libgstbase libprintbackend liborc liblcms libudev libunwind \
libEGL.so libGLX libwayland libgudev libgbm flatpak libxmlb.so libyaml $lib_audio_keywords $lib_browser_launcher"
...
bind_dirs=" /media /mnt /opt /run/media /usr/lib/locale /usr/share/fonts /usr/share/themes /var /usr/share/dbus-1 /usr/share/flatpak"
...
_enable_mountpoints_for_the_inbuilt_bubblewrap() {
	...
	mkdir -p ./"$APP".AppDir/.junest/usr/share/flatpak
